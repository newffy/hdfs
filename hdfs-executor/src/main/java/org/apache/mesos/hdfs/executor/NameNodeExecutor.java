...
public class NameNodeExecutor extends AbstractNodeExecutor {
  ...
  private void ensureStatusPathExists() {
    try {
      EnsurePath ensurePath = new EnsurePath(getStatusPath());
      ensurePath.ensure(curatorClient.getZookeeperClient());
    } catch (Exception ex) {
      log.error("Failed to ensure existence of path: " + getStatusPath() + " with exception: " + ex);
    }
  }
