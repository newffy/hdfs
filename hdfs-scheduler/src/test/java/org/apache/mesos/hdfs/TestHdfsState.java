...
public class TestHdfsState {
  ...
  private final Log log = LogFactory.getLog(TestHdfsState.class);
  ...
  @Test
  public void testSetFrameworkId()
    ...
    FrameworkID expectedId = createFrameworkId();
    state.setFrameworkId(expectedId);
    FrameworkID actualId = state.getFrameworkId();
  ...
  public void testRecordTask()
    ...
    assertEquals(inTask.getStatus(), outTask.getStatus());
    assertEquals(inTask.getOffer(), outTask.getOffer());
    assertEquals(inTask.getType(), outTask.getType());
    assertEquals(inTask.getName(), outTask.getName());
  }


  @Test
  public void testUpdateNoneToLabelsTask()
    throws ClassNotFoundException, IOException, InterruptedException, ExecutionException {
    HdfsState state = injector.getInstance(HdfsState.class);
    List<Task> tasks = state.getTasks();
    assertEquals(0, tasks.size());
    ...
    TaskStatus status = createTaskStatusWithLabel(inTask.getId(), TaskState.TASK_RUNNING, "value");
  ...
  @Test
  public void testUpdateLabelToLabelTask()
    throws ClassNotFoundException, IOException, InterruptedException, ExecutionException {
    HdfsState state = injector.getInstance(HdfsState.class);
    List<Task> tasks = state.getTasks();
    assertEquals(0, tasks.size());
    ...
    TaskStatus status1 = createTaskStatusWithLabel(task1.getId(), TaskState.TASK_RUNNING, "value1");
    TaskStatus status2 = createTaskStatusWithLabel(task1.getId(), TaskState.TASK_RUNNING, "value2");
    ...
    assertEquals(status2, outTask.getStatus());
  }

  @Test
  public void testUpdateLabelToNoneTask()
    throws ClassNotFoundException, IOException, InterruptedException, ExecutionException {
    HdfsState state = injector.getInstance(HdfsState.class);
    List<Task> tasks = state.getTasks();
    assertEquals(0, tasks.size());
    ...
    Task task1 = createTask();
    TaskStatus status1 = createTaskStatusWithLabel(task1.getId(), TaskState.TASK_RUNNING, "value1");
    TaskStatus status2 = createTaskStatus(task1.getId(), TaskState.TASK_RUNNING);
    ...
    assertEquals(status1, outTask.getStatus());
  }
